...
void func_ovl2_800E1260(GObj *fighter_gobj)
{
    ...
    if ((this_fp->item_hold != NULL) && (this_fp->status_info.status_id != ftStatus_Common_LightGet) && (atGetStruct(this_fp->item_hold)->at_kind == At_Kind_Hammer))
    {
...
void func_ovl2_800E2D44(Fighter_Struct *attacker_fp, Fighter_Hit *attacker_hit, Fighter_Struct *victim_fp, Fighter_Hurt *victim_hurt, GObj *attacker_gobj, GObj *victim_gobj)
{
    ...
    )
    {
        if (func_ovl2_800E2CC0(victim_fp, &damage) != FALSE)
        {
            ...
            ftCommon_AttackUpdateMatchStats(attacker_port_id, victim_fp->port_id, damage);
            ftCommon_AttackAddStaleQueue(attacker_port_id, victim_fp->port_id, attacker_hit->attack_id, attacker_hit->flags_hi.halfword);
        }
        else
        {
...
void func_ovl2_800E3048(Item_Struct *ip, Item_Hit *it_hit, s32 arg2, Fighter_Struct *fp, void *arg4, GObj *fighter_gobj, f32 angle, f32 *lr)
{
    s32 damage = func_ovl3_80168128(ip);
    Vec3f sp30;
...
void func_ovl2_800E3418(Item_Struct *ip, Item_Hit *it_hit, s32 arg2, Fighter_Struct *fp, Fighter_Hurt *ft_hurt, GObj *item_gobj, GObj *fighter_gobj)
{
    ...
    )
    {
        ...
        ftCommon_AttackUpdateMatchStats(ip->port_id, fp->port_id, damage);
        ftCommon_AttackAddStaleQueue(ip->port_id, fp->port_id, it_hit->attack_id, it_hit->motion_count);
    }
...
void func_ovl2_800E35BC(Article_Struct *ap, Article_Hit *at_hit, s32 arg2, Fighter_Struct *fp, Fighter_Hit *ft_hit, GObj *article_gobj, GObj *fighter_gobj)
{
    ...
    if ((damage - 10) < ft_hit->damage)
    {
        func_ovl3_8016F930(at_hit, fighter_gobj, gmHitCollision_Type_Hit, ft_hit->group_id);
...
void func_ovl2_800E39B0(Article_Struct *ap, Article_Hit *at_hit, s32 arg2, Fighter_Struct *fp, Fighter_Hurt *ft_hurt, GObj *article_gobj, GObj *fighter_gobj)
{
    ...
    else
    {
        ...
        )
        {
            ...
            ftCommon_AttackUpdateMatchStats(ap->port_id, fp->port_id, damage_again);
            ftCommon_AttackAddStaleQueue(ap->port_id, fp->port_id, at_hit->attack_id, at_hit->stat_count);
        }
...
void func_ovl2_800E3CAC(GObj *special_gobj, GObj *fighter_gobj, Fighter_Struct *fp, Ground_Hit *gr_hit, s32 target_kind)
{
    ...
    case 1:
        if (temp_v0 != 0)
        {
            ftCommon_AttackUpdateMatchStats(atGetStruct(special_gobj)->damage_port, fp->port_id, damage);
        }
        break;
...
void func_ovl2_800E3EBC(GObj *fighter_gobj)
{
    ...
    for (i = 0; i < ftHitCollisionLogIndex; i++)
    {
        ...
        case ftHitCollision_LogKind_Article:
            ...
            ap = atGetStruct(hitlog->attacker_gobj);
        ...
        case ftHitCollision_LogKind_Special:
            ...
            {
                gr_handicap = atGetStruct(hitlog->attacker_gobj)->damage_handicap;
            }
            else gr_handicap = 9;
    ...
    case ftHitCollision_LogKind_Article:
        ...
        if (this_fp->port_id == hitlog->attacker_port_id)
        {
            ...
            ftCommon_Update1PGameDamageStats(this_fp, GMMATCH_PLAYERS_MAX, hitlog->hit_source, ap->at_kind, 0, 0);
        }
        else
        {
            this_fp->damage_player_number = hitlog->attacker_player_number;
            ftCommon_Update1PGameDamageStats(this_fp, hitlog->attacker_port_id, hitlog->hit_source, ap->at_kind, at_hit->stat_flags.halfword, at_hit->stat_count);
        }
        this_fp->damage_joint_index = hitlog->victim_hurt->joint_index;
        this_fp->damage_index = hitlog->victim_hurt->unk_ftht_0xC;
...
void func_ovl2_800E61EC(GObj *fighter_gobj)
{
    ...
    if (fp->damage_knockback != 0.0F)
    {
        ...
        ftCommon_DamageUpdateCheckDropItem(fp, fp->damage_taken_recent);
    ...
    if ((hitlag_timer == 0) && (fp->afterimage.render_state != -1))
    {
        ...
        case TRUE:
            ...
            if ((fp->item_hold != NULL) && (fp->x190_flag_b6) && (atGetStruct(fp->item_hold)->at_kind == At_Kind_Sword))
            {
                s32 unused;
                HAL_Bitmap bitmap;
...
void func_ovl2_800E6F24(GObj *fighter_gobj, s32 status_id, f32 frame_begin, f32 anim_rate, u32 flags)
{
    ...
    if (fp->status_info.pl_kind != Pl_Kind_Result)
    {
        ...
        {
            ftCommon_MotionCountIncSetID(fp, status_struct[status_struct_id].flags_h.flags_hi_0x3F);
        }
