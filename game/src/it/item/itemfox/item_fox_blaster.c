...
{
    if (JObjGetStruct(item_gobj)->scale.x < 53.333332F)
    {
        JObjGetStruct(item_gobj)->scale.x += 5.3333335F;

        if (53.333332F < JObjGetStruct(item_gobj)->scale.x)
        {
            JObjGetStruct(item_gobj)->scale.x = (f32)53.333332F;
        }
    }
    return FALSE;
}
...
bool32 jtgt_ovl3_80168924(GObj *item_gobj)
{
    ...
    {
        func_ovl2_80103320(&JObjGetStruct(item_gobj)->translate);
