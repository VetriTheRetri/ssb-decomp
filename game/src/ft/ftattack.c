...
{
    s32 stale_index = Match_Info->player_block[player_id].stale_index;
    s32 backup_stale_id;
    s32 stale_id;
    s32 flag_id;
    ...
    if (attack_id != 0)
    {
        ...
        {
            if (attack_id == Match_Info->player_block[player_id].stale_flags[flag_id][0])
            {
                if (flags != Match_Info->player_block[player_id].stale_flags[flag_id][1])
                {
