...
next:
    ...
    {
        Fighter_Struct *attacker_fp = ftGetStruct(attacker_gobj);
...
bool32 ftCommon_WallDamage_CheckGoto(GObj *fighter_gobj)
{
    ...
    if (fp->status_vars.common.damage.coll_mask & MPCOLL_MASK_LWALL)
    {
    ...
    else if (fp->status_vars.common.damage.coll_mask & MPCOLL_MASK_RWALL)
    {
        pos.x -= fp->coll_data.object_coll.width;
        pos.y += fp->coll_data.object_coll.center;
